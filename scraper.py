...
def extract_goalscorers(soup):
    ...
    for table in soup.find_all("table"):
        for row in table.find_all("tr"):
            ...
            # The BBC scorer rows contain the rank first, followed by
            # Goals, Assists, Played, etc. The goal total is therefore
            # the second integer in the row.
...
def main():
    ...
    # ---------------------------------------------------------
    scorer_response = requests.get(SCORERS_URL, headers=HEADERS, timeout=30)
    scorer_response.raise_for_status()
    scorer_soup = BeautifulSoup(scorer_response.text, "html.parser")
    scorers = extract_goalscorers(scorer_soup)

    missing_scorers = [name for name in GOALSCORERS if name not in scorers]
    if missing_scorers:
        raise RuntimeError(
            "Could not find these goalscorers on BBC: "
